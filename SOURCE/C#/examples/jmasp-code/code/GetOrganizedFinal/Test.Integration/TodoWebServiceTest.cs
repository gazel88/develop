...
namespace Test.Integration
{
    ...
    public class TodoWebServiceTest
    {
        ...
        {
            return WebServiceUtil.AuthenticateWithForms("jonathan", "pass@word1",
        ...
        {
            HttpWebResponse response = Authenticate();
        ...
        {
            HttpWebResponse authenticateResponse = Authenticate();

            HttpWebResponse responseWithXML =
                WebServiceUtil.SendWebRequest("http://localhost:1901/Todo/", string.Empty,
                                              "text/xml", authenticateResponse.Cookies[0]);

            XmlReader xmlReader = XmlReader.Create(responseWithXML.GetResponseStream());
        ...
        public void Should_Send_Xml_To_Create_Todo()
        {
            ...
            HttpWebResponse authRequest = Authenticate();
            HttpWebResponse xmlRequest = WebServiceUtil.SendWebRequest("http://localhost:1901/Todo/CreateWithXml",
                                                                       xml, "text/xml", authRequest.Cookies[0]);

            XmlReader xmlReader = XmlReader.Create(xmlRequest.GetResponseStream());
            XDocument todoDoc = XDocument.Load(xmlReader);
